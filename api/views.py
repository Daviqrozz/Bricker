...
def product_view(request):
    ...
    if start_date:
        ...
        # Investimento = custo dos itens disponíveis cadastrados no período
        total_spent = relevant_products.filter(
        ...
        total_spent = Product.objects.filter(
            status=Product.Status.AVAILABLE
        ).aggregate(total=Sum('cost'))['total'] or 0

    total_sales = recent_sales.aggregate(total=Sum('price_sold'))['total'] or 0

    # Custo total dos itens vendidos no período
    total_cost = sum(sale.product.cost for sale in recent_sales)

    last_month_balance = total_sales - total_cost

    # Estoque = contagem de itens disponíveis
    stock_count = Product.objects.filter(status=Product.Status.AVAILABLE).count()
